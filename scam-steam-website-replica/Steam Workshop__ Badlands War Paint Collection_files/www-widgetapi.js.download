...
;var Ta,O;a:{for(var Ua=["CLOSURE_FLAGS"],P=E,Va=0;Va<Ua.length;Va++)if(P=P[Ua[Va]],P==null){O=null;break a}O=P}var Wa=O&&O[610401301];Ta=Wa!=null?Wa:!1;function Xa(){var a=E.navigator;return a&&(a=a.userAgent)?a:""}
var Q,Ya=E.navigator;Q=Ya?Ya.userAgentData||null:null;function Za(){return Ta?Q?Q.brands.some(function(a){return(a=a.brand)&&a.indexOf("Chromium")!=-1}):!1:!1}
...
;var eb;function fb(){var a=E.MessageChannel;typeof a==="undefined"&&typeof window!=="undefined"&&window.postMessage&&window.addEventListener&&!R("Presto")&&(a=function(){var f=db();f.style.display="none";document.documentElement.appendChild(f);var k=f.contentWindow;f=k.document;f.open();f.close();var e="callImmediate"+Math.random(),g=k.location.protocol=="file:"?"*":k.location.protocol+"//"+k.location.host;f=G(function(h){if((g=="*"||h.origin==g)&&h.data==e)this.port1.onmessage()},this);
...
function yb(){return qa(function(a){var b=a.return;var c=cb()>=119;return b.call(a,c&&!!navigator.permissions&&!!navigator.permissions.query&&"requestStorageAccessFor"in document)})}
function zb(){var a=new xb,b=["https://www.youtube.com"];b=b===void 0?wb:b;qa(function(c){switch(c.g){case 1:return ka(c,yb(),2);case 2:if(!c.m){c.g=3;break}return ka(c,Promise.all(b.map(function(d){var f;return qa(function(k){if(k.g==1)return k.l=2,ka(k,navigator.permissions.query({name:"top-level-storage-access",requestedOrigin:d}),4);k.g!=2?(f=k.m,f.state==="prompt"&&a.g.push(d),k.g=0,k.l=0):(k.l=0,k.i=null,k.g=0)})})),4);
...
function Gb(a,b,c){Y||(Y={},Hb=new Set,Ib.addEventListener("message",function(d){a:if(Hb.has(d.origin)){try{var f=JSON.parse(d.data)}catch(g){break a}var k=Y[f.id];if(k&&d.origin===k.O)switch(d=k.U,d.s=!0,d.s&&(I(d.m,d.sendMessage,d),d.m.length=0),k=f.event,f=f.info,k){case "apiInfoDelivery":if(F(f))for(var e in f)f.hasOwnProperty(e)&&(d.v[e]=f[e]);break;case "infoDelivery":Jb(d,f);break;case "initialDelivery":F(f)&&(clearInterval(d.i),d.playerInfo={},d.v={},Kb(d,f.apiInterface),Jb(d,f));break;default:d.j.i||
...
new Fb(b),c||(b=Ob(this,a),this.l=a,(c=a.parentNode)&&c.replaceChild(b,a),a=b),this.g=a,this.g.id||(this.g.id="widget"+sa(this.g)),W[this.g.id]=this,window.postMessage){this.j=new U;Pb(this);b=X(this.h,"events");for(var d in b)b.hasOwnProperty(d)&&this.addEventListener(d,b[d]);for(var f in Bb)Bb.hasOwnProperty(f)&&Qb(this,f)}}
...
function Tb(a){var b=X(a.h,"playerVars");if(b){var c={},d;for(d in b)c[d]=b[d];b=c}else b={};window!==window.top&&document.referrer&&(b.widget_referrer=document.referrer.substring(0,256));if(a=X(a.h,"embedConfig")){if(F(a))try{a=JSON.stringify(a)}catch(f){console.error("Invalid embed config JSON",f)}b.embed_config=a}return b}
